import * as obraSocialService from '../services/obrasSociales.service.js';

export const listarObrasSocialesController = async (req, res) => {
  try {
    const obrasSociales = await obraSocialService.listarObrasSociales();
...
export const crearObraSocialController = async (req, res) => {
  try {
    const id_obra_social = await obraSocialService.crearObraSocial(req.body);
...
export const editarObraSocialController = async (req, res) => {
  try {
    await obraSocialService.editarObraSocial(req.params.id, req.body);
...
export const asociarMedicoObraSocialController = async (req, res) => {
  try {
    ...
    const resultado = await obraSocialService.asociarMedicoObraSocial(id_medico, parseInt(id));
    return res.status(201).json({
      ok: true,
...
export const asociarPacienteObraSocialController = async (req, res) => {
  try {
    ...
    const resultado = await obraSocialService.asociarPacienteObraSocial(id_paciente, id);
